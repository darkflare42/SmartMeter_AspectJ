...
public aspect UpdateCurrentBillAspect {
    ...
    pointcut MeterReadingChanged() : execution(* PowerMeter.readWattage());


    pointcut CountryTariffChanged(int countryID, int newTariff) :
                                execution(* DBComm.changeCountryTariff(int, int)) && args(countryID, newTariff);


    pointcut FineCustomer() : execution(* BillingEngine.checkMonthlyBilling());

    pointcut CityTariffChanged(int countryID, int cityID, int newTariff) :
                                execution(* DBComm.changeCityTariff(int, int, int)) &&
                                args(countryID, cityID, newTariff);
